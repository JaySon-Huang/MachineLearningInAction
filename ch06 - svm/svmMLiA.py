...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):
    dataMatrix = mat(dataMatIn); labels = mat(classLabels).transpose()
    ...
    while iter < maxIter:
        alphaPairsChanged = 0
        for i in range(m):
            fXi = float(multiply(alphas,labels).T*(dataMatrix*dataMatrix[i,:].T)) + b
            ...
            if ((labels[i]*Ei < -toler) and (alphas[i] < C)) or ((labels[i]*Ei > toler) and (alphas[i] > 0)):
                j = selectJrand(i,m)
                fXj = float(multiply(alphas,labels).T*(dataMatrix*dataMatrix[j,:].T)) + b
                ...
                b1 = b - Ei- labels[i]*(alphas[i]-alphaIold)*dataMatrix[i,:]*dataMatrix[i,:].T - labels[j]*(alphas[j]-alphaJold)*dataMatrix[i,:]*dataMatrix[j,:].T
                b2 = b - Ej- labels[i]*(alphas[i]-alphaIold)*dataMatrix[i,:]*dataMatrix[j,:].T - labels[j]*(alphas[j]-alphaJold)*dataMatrix[j,:]*dataMatrix[j,:].T
